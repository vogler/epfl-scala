...
trait Solver extends GameDef {
   ...
   */
  def done(b: Block): Boolean = b.b1 == goal && b.b2 == goal // uses auto-generated equals for case classes, which checks for structural equality
  ...
  def from(initial: Stream[(Block, List[Move])],
	  ...
	  def r = initial.map(x => {
		  def b = x._1; def h = x._2;
		  def nwh = neighborsWithHistory(b, h);
		  def nno = newNeighborsOnly(nwh, explored.+(b));
		  nno
	  }).flatten;
      if(r==initial) r else r++from(r, explored++initial.map(_._1))
  }
