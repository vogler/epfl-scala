...
abstract class TweetSet {
  ...
  def filter0(p: Tweet => Boolean, accu: TweetSet): TweetSet = this match {
    case e: Empty => accu
    case n: NonEmpty => tail.filter0(p, if (p(head)) accu incl head else accu)
  }
...
//    val c = (left.filter(p) union right.filter(p))
//    if (p(elem)) c.incl(elem) else c
  
//    val a = if (p(elem)) accu incl elem else accu
//    left.filter0(p, a) union right.filter0(p, a)
  
//	val a = if (p(head)) accu incl head else accu
//    tail.filter0(p, a)
//  }
  ...
  def ascendingByRetweet0(accu: Trending): Trending = {
    ...
    val nts = this.remove(min)
    nts.ascendingByRetweet0(accu+min)
  }
...
//  val googleTweets: TweetSet = all.filter(t => t.text.contains(google))
  val googleTweets: TweetSet = all.filter(t => google.exists(s => t.text.contains(s)))

  val appleTweets: TweetSet = all.filter(t => apple.exists(s => t.text.contains(s)))
